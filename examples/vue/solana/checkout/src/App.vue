...
const solanaRpcUrl = import.meta.env.VITE_SOLANA_RPC_CONNECTION as string
const connection = new Connection(solanaRpcUrl)
const keypair = Keypair.fromSecretKey(
  Uint8Array.from([
    56, 14, 246, 208, 184, 212, 110, 37, 164, 242, 149, 110, 238, 115, 65, 243, 91, 159, 62, 192,
    243, 4, 84, 48, 19, 210, 63, 37, 208, 36, 158, 61, 242, 18, 206, 253, 243, 8, 213, 146, 146,
    126, 125, 217, 207, 28, 77, 218, 206, 8, 210, 3, 70, 26, 82, 213, 209, 139, 197, 120, 5, 103,
    102, 171
  ])
)
...
const usdcContractAddress = import.meta.env.VITE_CONTRACT_ADDRESS as string
...
const initializeTx = async () => {
  const fromTokenAccount = await connection.getParsedTokenAccountsByOwner(keypair.publicKey, {
    mint: USDC_MINT_ADDRESS
  })
  const toTokenAccount = await connection.getParsedTokenAccountsByOwner(recipientAddress, {
    mint: USDC_MINT_ADDRESS
  })
